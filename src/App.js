...
class Game {
  ...
  upgradeCoop = () => {
    ...
    this.upgradePrice = this.upgradePrice * 4;
    this.cigKofteFee += 10;
    this.maxPrice += 10;
    this.sellIndex += 1;
...
if(game.level > 1) {
  game.upgradePrice = game.upgradePrice * Math.pow(4, game.level - 1);
  ...
  get(ref(db, "APP/USERS")).then((snapshot) => {
    ...
    set(ref(db, "APP/USERS"), users).then(() => {
      ...
      sortedUsers.map((user, index) => {
        ...
        <div class="item-name">${user.title} (${user.level ? user.level : "1"} Level) <span class="item-start-date">(${user.createDate[0]}.${user.createDate[1] + 1}.${user.createDate[2]} ${user.createDate[3]}:${user.createDate[4]})</span></div>
