...
check = () => {
   const changeActivityBtns = document.querySelectorAll('.changeAutoBuyerActivity');
   ...
   if(Math.random() * game.cigKofteFee < 5 && game.currentCigKofte > 0) {
    ...
    if(game.hasAutoBuyer && game.isAutoBuyerActive && game.money >= game.newMaterialCost && game.material <= game.cigKofteMaterialAmount * (helpers.currentErrandBoy + helpers.currentMaster)) {
      game.buyMaterial();
    }
...
initLocalStorage = () => {
  ...
  if(localStorage.getItem('hasErrandBoy')) {
    ...
    helpers.currentErrandBoy = JSON.parse(localStorage.getItem('currentErrandBoy'));
    helpers.errandBoysPrice = JSON.parse(localStorage.getItem('errandBoysPrice'));
  ...
  if(localStorage.getItem('hasAutoBuyer')) {
    ...
    game.isAutoBuyerActive = JSON.parse(localStorage.getItem('isAutoBuyerActive'));
...
resetGame = (e) => {
  ...
  if(confirm("Are you sure about reset the game?")) {
    // reset local storage
    localStorage.clear();
